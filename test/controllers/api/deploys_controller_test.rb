...
module Shipit
  module Api
    class DeploysControllerTest < ActionController::TestCase
      ...
      test "#create_status renders a 422 if status is not found" do
        @stack = shipit_stacks(:shipit_canaries)
        @deploy = shipit_deploys(:canaries_validating)

        post :create_status, params: { stack_id: @stack.to_param, deploy_id: @deploy.id }
        assert_response :unprocessable_entity
        assert_json 'errors', 'status' => ['is required']
      end

      test "#create_status renders a 422 if status is invalid" do
        @stack = shipit_stacks(:shipit_canaries)
        @deploy = shipit_deploys(:canaries_validating)

        assert_no_difference -> { ReleaseStatus.count } do
          post :create_status, params: {
      ...
      test "#create_status allow users to append release statuses and mark the deploy as success" do
        @stack = shipit_stacks(:shipit_canaries)
        @deploy = shipit_deploys(:canaries_validating)
      ...
      test "#create_status allow users to append release statuses and mark the deploy as faulty" do
        @stack = shipit_stacks(:shipit_canaries)
        @deploy = shipit_deploys(:canaries_validating)
